...
contract TokenManager is System, IApplication, IParamSubscriber {
    ...
    function decodeBindSynPackage(bytes memory msgBytes) internal pure returns (BindSynPackage memory, bool) {
        ...
        while (iter.hasNext()) {
            if (idx == 0) {
                bindSynPkg.packageType = uint8(iter.next().toUint());
            } else if (idx == 1) {
                bindSynPkg.bep2TokenSymbol = bytes32(iter.next().toUint());
            } else if (idx == 2) {
                bindSynPkg.contractAddr = iter.next().toAddress();
            } else if (idx == 3) {
                bindSynPkg.totalSupply = iter.next().toUint();
            } else if (idx == 4) {
                bindSynPkg.peggyAmount = iter.next().toUint();
            } else if (idx == 5) {
                bindSynPkg.bep20Decimals = uint8(iter.next().toUint());
            } else if (idx == 6) {
    ...
    function encodeReactBindSynPackage(ReactBindSynPackage memory reactBindSynPackage)
        internal
        pure
    ...
    function approveBind(address contractAddr, string memory bep2Symbol) public payable returns (bool) {
        ...
        require(
            IBEP20(contractAddr).allowance(msg.sender, address(this)).add(tokenHubBalance) >= lockedAmount,
        ...
        if (verifyCode == CODE_OK) {
            ...
            ITokenHub(TOKEN_HUB_ADDR).bindToken(
                bindSynPkg.bep2TokenSymbol, bindSynPkg.contractAddr, bindSynPkg.bep20Decimals
            );
        ...
        ReactBindSynPackage memory reactBindSynPackage =
            ReactBindSynPackage({ status: verifyCode, bep2TokenSymbol: bep2TokenSymbol });
        address(uint160(TOKEN_HUB_ADDR)).transfer(relayFee);
        ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
            BIND_CHANNELID, encodeReactBindSynPackage(reactBindSynPackage), relayFee.div(TEN_DECIMALS)
        );
        return true;
    }
    ...
    function rejectBind(address contractAddr, string memory bep2Symbol) public payable returns (bool) {
        ...
        delete bindPackageRecord[bep2TokenSymbol];
        ReactBindSynPackage memory reactBindSynPackage =
            ReactBindSynPackage({ status: BIND_STATUS_REJECTED, bep2TokenSymbol: bep2TokenSymbol });
        address(uint160(TOKEN_HUB_ADDR)).transfer(relayFee);
        ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
            BIND_CHANNELID, encodeReactBindSynPackage(reactBindSynPackage), relayFee.div(TEN_DECIMALS)
        );
    ...
    function expireBind(string memory bep2Symbol) public payable returns (bool) {
        ...
        delete bindPackageRecord[bep2TokenSymbol];
        ReactBindSynPackage memory reactBindSynPackage =
            ReactBindSynPackage({ status: BIND_STATUS_TIMEOUT, bep2TokenSymbol: bep2TokenSymbol });
        address(uint160(TOKEN_HUB_ADDR)).transfer(relayFee);
        ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
            BIND_CHANNELID, encodeReactBindSynPackage(reactBindSynPackage), relayFee.div(TEN_DECIMALS)
        );
    ...
    function decodeMirrorSynPackage(bytes memory msgBytes) internal pure returns (MirrorSynPackage memory, bool) {
        ...
        while (iter.hasNext()) {
            if (idx == 0) {
                mirrorSynPackage.mirrorSender = iter.next().toAddress();
            } else if (idx == 1) {
                mirrorSynPackage.bep20Addr = iter.next().toAddress();
            } else if (idx == 2) {
                mirrorSynPackage.bep20Name = bytes32(iter.next().toUint());
            } else if (idx == 3) {
                mirrorSynPackage.bep20Symbol = bytes32(iter.next().toUint());
            } else if (idx == 4) {
                mirrorSynPackage.bep20Supply = iter.next().toUint();
            } else if (idx == 5) {
                mirrorSynPackage.bep20Decimals = uint8(iter.next().toUint());
            } else if (idx == 6) {
                mirrorSynPackage.mirrorFee = iter.next().toUint();
            } else if (idx == 7) {
    ...
    function decodeMirrorAckPackage(bytes memory msgBytes) internal pure returns (MirrorAckPackage memory, bool) {
        ...
        while (iter.hasNext()) {
            if (idx == 0) {
                mirrorAckPackage.mirrorSender = iter.next().toAddress();
            } else if (idx == 1) {
                mirrorAckPackage.bep20Addr = iter.next().toAddress();
            } else if (idx == 2) {
                mirrorAckPackage.bep20Decimals = uint8(iter.next().toUint());
            } else if (idx == 3) {
                mirrorAckPackage.bep2Symbol = bytes32(iter.next().toUint());
            } else if (idx == 4) {
                mirrorAckPackage.mirrorFee = iter.next().toUint();
            } else if (idx == 5) {
    ...
    function mirror(address bep20Addr, uint64 expireTime) public payable returns (bool) {
        ...
        require(
            symbolBytes.length >= MINIMUM_BEP20_SYMBOL_LEN && symbolBytes.length <= MAXIMUM_BEP20_SYMBOL_LEN,
            "symbol length must be in [2,8]"
        );
        for (uint8 i = 0; i < symbolBytes.length; ++i) {
            require(
                (symbolBytes[i] >= "A" && symbolBytes[i] <= "Z") || (symbolBytes[i] >= "a" && symbolBytes[i] <= "z")
        ...
        ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
            MIRROR_CHANNELID, encodeMirrorSynPackage(mirrorSynPackage), msg.value.sub(mirrorFee).div(TEN_DECIMALS)
        );
        return true;
    }
    ...
    function handleMirrorAckPackage(bytes memory msgBytes) internal {
        ...
        if (mirrorAckPackage.errorCode == CODE_OK) {
            ...
            ITokenHub(TOKEN_HUB_ADDR).bindToken(
                mirrorAckPackage.bep2Symbol, mirrorAckPackage.bep20Addr, mirrorAckPackage.bep20Decimals
            );
    ...
    function decodeSyncSynPackage(bytes memory msgBytes) internal pure returns (SyncSynPackage memory, bool) {
        ...
        while (iter.hasNext()) {
            if (idx == 0) {
                syncSynPackage.syncSender = iter.next().toAddress();
            } else if (idx == 1) {
                syncSynPackage.bep20Addr = iter.next().toAddress();
            } else if (idx == 2) {
                syncSynPackage.bep2Symbol = bytes32(iter.next().toUint());
            } else if (idx == 3) {
                syncSynPackage.bep20Supply = iter.next().toUint();
            } else if (idx == 4) {
                syncSynPackage.syncFee = iter.next().toUint();
            } else if (idx == 5) {
    ...
    function decodeSyncAckPackage(bytes memory msgBytes) internal pure returns (SyncAckPackage memory, bool) {
        ...
        while (iter.hasNext()) {
            if (idx == 0) {
                syncAckPackage.syncSender = iter.next().toAddress();
            } else if (idx == 1) {
                syncAckPackage.bep20Addr = iter.next().toAddress();
            } else if (idx == 2) {
                syncAckPackage.syncFee = iter.next().toUint();
            } else if (idx == 3) {
    ...
    function sync(address bep20Addr, uint64 expireTime) public payable returns (bool) {
        ...
        ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
            SYNC_CHANNELID, encodeSyncSynPackage(syncSynPackage), msg.value.sub(syncFee).div(TEN_DECIMALS)
        );
        return true;
    }
    ...
    function updateParam(string calldata key, bytes calldata value) external override onlyGov {
        ...
        } else if (bytes32Key == bytes32(0x73796e6346656500000000000000000000000000000000000000000000000000)) {
            // syncFee
            uint256 newSyncFee;
            assembly {
