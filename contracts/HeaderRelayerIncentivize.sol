...
contract HeaderRelayerIncentivize is IHeaderRelayerIncentivize {
  ...
  event LogAddReward(address headerRelayerAddr, uint256 amount);
  ...
  function addReward(address payable headerRelayerAddr, address payable caller) external override payable returns (bool) {
    ...
    if (_relayersSubmitCount[_roundSequence][headerRelayerAddr]==0){
      _relayerAddressRecord[_roundSequence].push(headerRelayerAddr);
    }
    _relayersSubmitCount[_roundSequence][headerRelayerAddr]++;
    emit LogAddReward(headerRelayerAddr, msg.value);

    if (_countInRound==roundSize){
      emit LogRewardPeriodExpire(_roundSequence, _collectedRewardRound[_roundSequence]);
      claimReward(_roundSequence, caller);
      _roundSequence++;
      _countInRound = 0;
    }
    return true;
  ...
  function claimReward(uint256 sequence, address payable caller) internal returns (bool) {
