...
contract TendermintLightClient is ILightClient {
    ...
    function isHeaderSynced(uint64 height) override external view returns (bool) {
        return _submitters[height] != address(0x0);
    }

    function getAppHash(uint64 height) override external view returns (bytes32) {
        return _BBCLightClientConsensusState[height].appHash;
    }

    function getSubmitter(uint64 height) override external view returns (address payable) {
