...
contract TendermintLightClient is ILightClient {
  ...
  function initConsensusState(bytes memory initConsensusStateBytes, string memory chain_id, address systemRewardContractAddr)
    public onlyNotInit {
    uint256 pointer;
    uint256 length;
  ...
  function syncTendermintHeader(bytes calldata header, uint64 height) external onlyAlreadyInit returns (bool) {
