...
contract BSCValidatorSet is IBSCValidatorSet, System, IParamSubscriber, Application {
  ...
  function updateValidatorSet(bytes memory validatorSetBytes) internal{
    ...
    if(crossTotal > 0){
      uint256 relayFee = crossAddrs.length*(syncRelayFee.add(ackRelayFee));
      try ITokenHub(TOKEN_HUB_ADDR).batchTransferOut{value:crossTotal}(crossAddrs, crossAmounts, crossRefundAddrs, address(0x0), block.timestamp + expireTimeSecondGap) returns (bool success) {
        if (success) {
