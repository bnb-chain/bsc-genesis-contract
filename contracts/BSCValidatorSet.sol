...
contract BSCValidatorSet is IBSCValidatorSet, System, IParamSubscriber, IApplication {
  ...
  uint32 public CODE_OK = 0;
  uint32 public ERROR_FAIL_DECODE = 101;
  uint32 public ERROR_UNKNOWN_PACKAGE_TYPE = 102;
  uint32 public ERROR_FAIL_CHECK_VALIDATORS = 103;
  uint32 public ERROR_LEN_OF_VAL_MISMATCH = 104;
  uint32 public ERROR_RELAYFEE_TOO_LARGE = 105;
  ...
  function handleSyncPackage(uint8, bytes calldata msgBytes) onlyInit onlyCrossChainContract external override returns(bytes memory responsePayload) {

    (IbcValidatorSetPackage memory validatorSetPackage, bool ok) = decodeValidatorSetSyncPackage(msgBytes);
    if (!ok){
      return ERROR_FAIL_DECODE.encodeCommonAckPackage();
    }
    uint32 resCode;
    ...
    return resCode.encodeCommonAckPackage();
  }
