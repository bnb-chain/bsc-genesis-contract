...
contract RelayerIncentivize is IRelayerIncentivize, System, IParamSubscriber {
    ...
    function addReward(
        ...
        if (fromSystemReward) {
            actualAmount = ISystemReward(SYSTEM_REWARD_ADDR).claimRewards(
                address(uint160(INCENTIVIZE_ADDR)), amount.add(dynamicExtraIncentiveAmount)
            );
        } else {
            ...
            if (dynamicExtraIncentiveAmount > 0) {
                actualAmount = actualAmount.add(
                    ISystemReward(SYSTEM_REWARD_ADDR).claimRewards(
                        address(uint160(INCENTIVIZE_ADDR)), dynamicExtraIncentiveAmount
                    )
                );
            }
        }
        ...
        if (countInRound >= ROUND_SIZE) {
            emit distributeCollectedReward(
                roundSequence, collectedRewardForHeaderRelayer, collectedRewardForTransferRelayer
            );
            ...
            relayerRewardVault[packageRelayer] =
                relayerRewardVault[packageRelayer].add(callerHeaderReward).add(callerPackageReward);
    ...
    function updateParam(string calldata key, bytes calldata value) external override onlyGov {
        ...
        if (Memory.compareStrings(key, "headerRelayerRewardRateMolecule")) {
            ...
            require(
                newHeaderRelayerRewardRateMolecule <= headerRelayerRewardRateDenominator,
                "new headerRelayerRewardRateMolecule shouldn't be greater than headerRelayerRewardRateDenominator"
            );
            ...
            require(
                newHeaderRelayerRewardRateDenominator != 0
                    && newHeaderRelayerRewardRateDenominator >= headerRelayerRewardRateMolecule,
                "the new headerRelayerRewardRateDenominator must not be zero and no less than headerRelayerRewardRateMolecule"
            );
            ...
            require(
                newCallerCompensationMolecule <= callerCompensationDenominator,
                "new callerCompensationMolecule shouldn't be greater than callerCompensationDenominator"
            );
            ...
            require(
                newCallerCompensationDenominator != 0 && newCallerCompensationDenominator >= callerCompensationMolecule,
                "the newCallerCompensationDenominator must not be zero and no less than callerCompensationMolecule"
            );
