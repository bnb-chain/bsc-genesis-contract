...
contract TransferRelayerIncentivize is ITransferRelayerIncentivize {
  ...
  function addReward(address payable relayerAddr) external override payable returns (bool) {
    ...
    if (_countInRound==roundSize){
      ...
      claimReward(_roundSequence, relayerAddr);
      _roundSequence++;
      _countInRound = 0;
    }
    return true;
  ...
  function claimReward(uint256 sequence, address payable relayerAddr) internal returns (bool) {
    ...
    relayerAddr.transfer(callerReward);
