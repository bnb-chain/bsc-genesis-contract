...
contract TokenHub is ITokenHub, System, IParamSubscriber, IApplication {
  ...
  function encodeBindAckPackage(BindAckPackage memory bindAckPackage) internal pure returns (bytes memory) {
    return uint256(bindAckPackage.bep2TokenSymbol).encodeUint();
  }
  ...
  function encodeTransferOutSyncPackage(TransferOutSyncPackage memory transOutSyncPkg) internal pure returns (bytes memory) {
    ...
    elements[1] = amountsElements.encodeList();
    ...
    elements[2] = recipientsElements.encodeList();
    ...
    elements[3] = refundAddrsElements.encodeList();

    elements[4] = uint256(transOutSyncPkg.expireTime).encodeUint();
    return elements.encodeList();
  }
  ...
  function transferOut(address contractAddr, address recipient, uint256 amount, uint64 expireTime) override external payable returns (bool) {
    ...
    ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendPackage(TRANSFER_OUT_CHANNELID, encodeTransferOutSyncPackage(transOutSyncPkg), syncRelayFee, ackRelayFee);
    return true;
  }
  ...
  function batchTransferOut(address[] calldata recipientAddrs, uint256[] calldata amounts, address[] calldata refundAddrs, address contractAddr, uint64 expireTime) override external payable returns (bool) {
    ...
    uint256 convertedTotalSyncRelayFee = syncRelayFee.mul(batchLength).div(1e10); // native bnb decimals is 8 on BBC, while the native bnb decimals on BSC is 18
    uint256 totalAckRelayFee = ackRelayFee.mul(batchLength); // native bnb decimals is 8 on BBC, while the native bnb decimals on BSC is 18
    ...
    ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendPackage(TRANSFER_OUT_CHANNELID, encodeTransferOutSyncPackage(transOutSyncPkg), syncRelayFee.mul(batchLength), ackRelayFee.mul(batchLength));
    return true;
  }
