...
contract SlashIndicator is ISlashIndicator,System,IParamSubscriber, IApplication{
  ...
  function submitFinalityViolationEvidence(FinalityEvidence calldata _evidence) external onlyInit onlyRelayer {
    ...
    // Basic check
    require(srcNumA+256 > block.number && srcNumB+256 > block.number, "too old block involved in the evidence");
    ...
    // Vote rules check
    if (!((srcNumA<srcNumB && tarNumB<tarNumA) || (srcNumB<srcNumA && tarNumA<tarNumB)) && tarNumA != tarNumB) {
