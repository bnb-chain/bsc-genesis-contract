...
contract RelayerIncentivizeTest is Deployer {
    ...
    function testHeaderRelay() public {
        ...
        for (uint256 i; i < 1; ++i) {
            incentivize.addReward(addrs[0], payable(address(crossChain)), 1e16, false);
        }
        for (uint256 i; i < 2; ++i) {
            incentivize.addReward(addrs[1], payable(address(crossChain)), 1e16, false);
        }
        for (uint256 i; i < 3; ++i) {
            incentivize.addReward(addrs[2], payable(address(crossChain)), 1e16, false);
        }
        for (uint256 i; i < 4; ++i) {
            incentivize.addReward(addrs[3], payable(address(crossChain)), 1e16, false);
        }
        for (uint256 i; i < 5; ++i) {
            incentivize.addReward(addrs[4], payable(address(crossChain)), 1e16, false);
        }
        for (uint256 i; i < 40; ++i) {
            incentivize.addReward(addrs[5], payable(address(crossChain)), 1e16, false);
        }
        for (uint256 i; i < 41; ++i) {
            incentivize.addReward(addrs[6], payable(address(crossChain)), 1e16, false);
        }
        for (uint256 i; i < 4; ++i) {
            incentivize.addReward(addrs[7], payable(address(crossChain)), 1e16, false);
        }
    ...
    function testPackageRelay() public {
        ...
        gas = gasleft();
        for (uint256 i; i < 1; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[0], 1e16, false);
        }
        txFees[0] = gasPrice * (gas - gasleft());
        gas = gasleft();
        for (uint256 i; i < 2; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[1], 1e16, false);
        }
        txFees[1] = gasPrice * (gas - gasleft());
        gas = gasleft();
        for (uint256 i; i < 3; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[2], 1e16, false);
        }
        txFees[2] = gasPrice * (gas - gasleft());
        gas = gasleft();
        for (uint256 i; i < 4; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[3], 1e16, false);
        }
        txFees[3] = gasPrice * (gas - gasleft());
        gas = gasleft();
        for (uint256 i; i < 5; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[4], 1e16, false);
        }
        txFees[4] = gasPrice * (gas - gasleft());
        gas = gasleft();
        for (uint256 i; i < 40; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[5], 1e16, false);
        }
        txFees[5] = gasPrice * (gas - gasleft());
        gas = gasleft();
        for (uint256 i; i < 41; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[6], 1e16, false);
        }
        txFees[6] = gasPrice * (gas - gasleft());
        gas = gasleft();
        for (uint256 i; i < 4; ++i) {
            incentivize.addReward(payable(address(crossChain)), addrs[7], 1e16, false);
        }
    ...
    function testNonPayableAddress() public {
        ...
        for (uint256 i; i < 50; ++i) {
            incentivize.addReward(payable(address(lightClient)), relayer, 1e16, false);
        }
        for (uint256 i; i < 49; ++i) {
            incentivize.addReward(relayer, payable(address(lightClient)), 1e16, false);
        }
        incentivize.addReward(relayer, payable(address(lightClient)), 1e16, false);
        ...
        assertEq(
            newBalanceSystemReward, balanceSystemReward + 506250000000000000, "wrong amount to systemReward contract"
        );
        assertEq(incentivize.roundSequence(), roundSeq + 1, "wrong sequence");
    ...
    function testDynamicExtraIncentive() public {
        ...
        for (uint256 i; i < roundSize; ++i) {
            incentivize.addReward(relayer, payable(address(crossChain)), 2e16, false);
        }
        ...
        vm.startPrank(address(crossChain));
        for (uint256 i; i < roundSize; ++i) {
            incentivize.addReward(relayer, payable(address(crossChain)), 1e16, false);
        }
        assertEq(incentivize.roundSequence(), ++roundSeq, "wrong sequence");
        incentivize.claimRelayerReward(relayer);
