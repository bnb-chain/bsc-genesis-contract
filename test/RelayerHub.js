...
contract("RelayerHub", (accounts) => {
  it("register and unregister success", async () => {
    ...
    assert.equal(
      web3.utils
        .toBN(balanceAfter)
        .sub(web3.utils.toBN(balanceBefore))
        .add(web3.utils.toBN(2e10).mul(web3.utils.toBN(tx.receipt.gasUsed)))
        .toString(),
      deposit.sub(dues).toString()
    );

    let systemRewardAfter = await web3.eth.getBalance(systemRewardInstance.address);
    assert.equal(
      web3.utils.toBN(systemRewardAfter).sub(web3.utils.toBN(systemRewardBefore)).toString(),
      web3.utils.toBN(1e17).toString()
    );
  });
