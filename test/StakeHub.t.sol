...
contract StakeHubTest is Deployer {
    ...
    function _createValidator(uint256 delegation) internal returns (address operatorAddress, address pool) {
        ...
        emit ValidatorCreated(consensusAddress, operatorAddress, address(0), blsPubKey);
        stakeHub.createValidator{value: delegation}(consensusAddress, blsPubKey, bytes(""), commission, description, vm.toString(operatorAddress));
