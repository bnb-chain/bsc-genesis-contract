...
contract("RelayerIncentivize", (accounts) => {
  ...
  it("header relayer incentivize", async () => {
    ...
    for (let i = 0; i < 1; i++) {
      await relayerIncentivize.addReward(accounts[1], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    for (let i = 0; i < 2; i++) {
      await relayerIncentivize.addReward(accounts[2], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    for (let i = 0; i < 3; i++) {
      await relayerIncentivize.addReward(accounts[3], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    for (let i = 0; i < 4; i++) {
      await relayerIncentivize.addReward(accounts[4], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    for (let i = 0; i < 5; i++) {
      await relayerIncentivize.addReward(accounts[5], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    for (let i = 0; i < 6; i++) {
      await relayerIncentivize.addReward(accounts[6], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    for (let i = 0; i < 7; i++) {
      await relayerIncentivize.addReward(accounts[7], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    for (let i = 0; i < 2; i++) {
      await relayerIncentivize.addReward(accounts[8], accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
  ...
  it("transfer relayer Incentivize", async () => {
    ...
    for (let i = 0; i < 1; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[1], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
    ...
    for (let i = 0; i < 2; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[2], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
      const txFee = web3.utils.toBN(tx.receipt.gasUsed).mul(gasPrice);
    ...
    for (let i = 0; i < 3; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[3], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
      const txFee = web3.utils.toBN(tx.receipt.gasUsed).mul(gasPrice);
    ...
    for (let i = 0; i < 4; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[4], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
      const txFee = web3.utils.toBN(tx.receipt.gasUsed).mul(gasPrice);
    ...
    for (let i = 0; i < 5; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[5], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
      const txFee = web3.utils.toBN(tx.receipt.gasUsed).mul(gasPrice);
    ...
    for (let i = 0; i < 6; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[6], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
      const txFee = web3.utils.toBN(tx.receipt.gasUsed).mul(gasPrice);
    ...
    for (let i = 0; i < 7; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[7], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
      const txFee = web3.utils.toBN(tx.receipt.gasUsed).mul(gasPrice);
    ...
    for (let i = 0; i < 2; i++) {
      let tx = await relayerIncentivize.addReward(accounts[0], accounts[8], web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
      const txFee = web3.utils.toBN(tx.receipt.gasUsed).mul(gasPrice);
    ...
    const pureRewardAccount1 = web3.utils
      .toBN(newAccount1Balance)
      .sub(web3.utils.toBN(initialAccount1Balance))
      .sub(account1TxFee);
    const pureRewardAccount2 = web3.utils
      .toBN(newAccount2Balance)
      .sub(web3.utils.toBN(initialAccount2Balance))
      .sub(account2TxFee);
    const pureRewardAccount3 = web3.utils
      .toBN(newAccount3Balance)
      .sub(web3.utils.toBN(initialAccount3Balance))
      .sub(account3TxFee);
    const pureRewardAccount4 = web3.utils
      .toBN(newAccount4Balance)
      .sub(web3.utils.toBN(initialAccount4Balance))
      .sub(account4TxFee);
    const pureRewardAccount5 = web3.utils
      .toBN(newAccount5Balance)
      .sub(web3.utils.toBN(initialAccount5Balance))
      .sub(account5TxFee);
    const pureRewardAccount6 = web3.utils
      .toBN(newAccount6Balance)
      .sub(web3.utils.toBN(initialAccount6Balance))
      .sub(account6TxFee);
    const pureRewardAccount7 = web3.utils
      .toBN(newAccount7Balance)
      .sub(web3.utils.toBN(initialAccount7Balance))
      .sub(account7TxFee);
    const pureRewardAccount8 = web3.utils
      .toBN(newAccount8Balance)
      .sub(web3.utils.toBN(initialAccount8Balance))
      .sub(account8TxFee);
  ...
  it("non-payable address", async () => {
    ...
    for (let i = 0; i < 15; i++) {
      await relayerIncentivize.addReward(tendermintLightClient.address, relayer, web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
    }

    for (let i = 0; i < 14; i++) {
      await relayerIncentivize.addReward(relayer, tendermintLightClient.address, web3.utils.toBN(1e16), false, {
        from: tokenHub,
      });
    }
    ...
    await relayerIncentivize.addReward(relayer, tendermintLightClient.address, web3.utils.toBN(1e16), true, {
      from: tokenHub,
    });
    ...
    assert.equal(
      web3.utils
        .toBN(newSystemRewardBalance)
        .sub(web3.utils.toBN(originSystemRewardBalance))
        .eq(web3.utils.toBN(146812500000000000)),
      true,
      "wrong amount to systemReward contract"
    );
  ...
  it("dynamic extra incentive", async () => {
    ...
    for (let i = 0; i < 30; i++) {
      await relayerIncentivize.addReward(relayer, accounts[0], web3.utils.toBN(1e16), false, {
        from: accounts[0],
      });
    }
    ...
    for (let i = 0; i < 30; i++) {
      await relayerIncentivize.addReward(relayer, accounts[0], web3.utils.toBN(2e16), false, {
        from: accounts[0],
      });
    }
    ...
    assert.equal(
      web3.utils.toBN(dynamicExtraIncentiveAmount).eq(web3.utils.toBN(0)),
      true,
      "wrong dynamicExtraIncentiveAmount"
    );
    ...
    assert.equal(
      web3.utils.toBN(dynamicExtraIncentiveAmount).eq(web3.utils.toBN(1e16)),
      true,
      "wrong dynamicExtraIncentiveAmount"
    );
