...
contract TokenHubTest is Deployer {
    ...
    function testBindMaliciousToken() public {
        bytes memory pack =
            buildBindPackage(uint8(0), bytes32("MALICIOU-A09"), address(maliciousToken), 1e8, 99e6, uint8(18));
        vm.prank(address(crossChain));
        tokenManager.handleSynPackage(BIND_CHANNELID, pack);
    ...
    function testMirrorAndSncSucceed() public {
        ...
        bytes memory pack =
            buildMirrorAckPackage(address(this), address(xyzToken), uint8(18), bytes32("XYZ-123"), mirrorFee, uint8(0));
        vm.prank(address(crossChain));
        tokenManager.handleAckPackage(MIRROR_CHANNELID, pack);
        assertEq(address(tokenManager).balance, 0, "wrong balance in tokenManager");
        ...
        // sync fail ack
        pack = buildSyncFailAckPackage(
            address(this), address(xyzToken), bytes32("XYZ"), totalSupply, syncFee / 1e10, expireTime
        );
        vm.prank(address(crossChain));
    ...
    function buildTransferInPackage(
        bytes32 symbol,
        address tokenAddr,
        uint256 amount,
        address recipient,
    ...
    function buildMirrorAckPackage(
        address sender,
        address tokenAddr,
        uint8 decimal,
        bytes32 symbol,
    ...
    function buildSyncFailAckPackage(
        address sender,
        address tokenAddr,
        bytes32 symbol,
        uint256 supply,
