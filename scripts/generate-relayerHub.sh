...
case $NETWORK in
local)
    if [ -n "${WHITELIST1_ADDRESS}" ]; then
        LOCAL_WHITELIST1="address public constant WHITELIST_1 = ${WHITELIST1_ADDRESS};"
    fi
    if [ -n "${WHITELIST2_ADDRESS}" ]; then
        LOCAL_WHITELIST2="address public constant WHITELIST_2 = ${WHITELIST2_ADDRESS};"
    fi
